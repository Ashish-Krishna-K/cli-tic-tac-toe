...
class GameManager:
    ...
    def __init__(self) -> None:
        ...
        self.__played_tiles: set[str] = set[str]()
    ...
    def get_opponent_pref(self) -> None:
        ...
        while user_choice not in ["p", "c"]:
            user_choice = input(
                "Do you want to play against another player or computer? (p/c) "
            ).lower()
            print(user_choice)
        player_name = "Computer" if user_choice == "c" else "Player 2"
    ...
    def computer_plays(self) -> int:
        ...
        while str(random_play) in self.__played_tiles:
    ...
    def add_played_tile(self, tile: str) -> None:
        self.__played_tiles.add(tile)
